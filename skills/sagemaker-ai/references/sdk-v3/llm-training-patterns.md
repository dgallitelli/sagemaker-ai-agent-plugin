# SageMaker SDK v3 — LLM Training Patterns

Training and fine-tuning patterns for LLMs using SageMaker Training Jobs, Serverless Model Customization, and HyperPod.
...
Trainium has limited architecture support. Verified: `llama`, `qwen3`, `granite`. Always check `model_type` in config.json and the Neuron SDK compatibility matrix. Variants like `qwen3_vl`, `mllama` are NOT supported.

### Quick Instance Reference (from AWS recipes repo)
...
Fully managed capability (launched December 2025) — fine-tune foundation models without infrastructure management.
...
- SFT, DPO, RLVR, RLAIF, LoRA/QLoRA, Full-Rank, CPT
